...
export default function Home() {
  ...
  // Google OAuth & Supabase authentication states
  ...
  // Simulated Google OAuth & Supabase DB connection sequence
  const handleGoogleSignIn = () => {
    sounds.click();
    setIsSigningIn(true);
    setSignInProgress(0);
    setSignInStatus(simpleEnglish ? 'CONTACTING SUPABASE AUTH...' : 'ESTABLISHING GOOGLE OAUTH SECURITY CHANNEL...');
    
    setTimeout(() => {
      setSignInProgress(35);
      setSignInStatus(simpleEnglish ? 'CHECKING DATABASE...' : 'SUPABASE CLUSTER SECURED. QUERYING POSTGRESQL...');
    }, 600);

    setTimeout(() => {
      setSignInProgress(70);
      setSignInStatus(simpleEnglish ? 'SIGNING YOU IN...' : 'SYNCHRONIZING USER LEDGER AND PERMISSIONS...');
    }, 1300);

    setTimeout(() => {
      setSignInProgress(100);
      setSignInStatus(simpleEnglish ? 'SUCCESS!' : 'AUTHENTICATION GRANTED. DIRECTING TO FLIGHT COCKPIT...');
    }, 2000);

    setTimeout(() => {
      setIsSigningIn(false);
      // Initialize onboarding user state
  ...
  return (
    <div className="relative min-h-screen text-slate-900 bg-[#faf9f6] overflow-x-hidden font-sans selection:bg-yellow-400 selection:text-black">
      ...
      {/* Google OAuth & Supabase authentication status gateway modal */}
      {isSigningIn && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4 select-none">
          <div className="absolute inset-0 pointer-events-none crt-glow scanlines opacity-[0.05] bg-yellow-500" />
          <div className="w-full max-w-md bg-slate-950 border-4 border-yellow-400 p-6 rounded-lg text-white shadow-[0_0_30px_rgba(234,179,8,0.25)] relative font-mono text-center">
            
            <div className="flex justify-center mb-4">
              <div className="h-10 w-10 border-4 border-yellow-400 border-t-transparent rounded-full animate-spin" />
            </div>

            <div className="text-[10px] text-yellow-400 font-bold uppercase tracking-widest mb-1.5 animate-pulse">
              &gt;&gt; SECURE_OAUTH_PROCESSOR
            </div>
            
            <h3 className="text-sm font-black tracking-wide uppercase text-white mb-6">
              {simpleEnglish ? "AUTHENTICATION SERVICE" : "SUPABASE SECURE CLUSTER AUTH"}
            </h3>

            {/* Progress bar */}
            <div className="w-full bg-slate-900 border-2 border-black h-6 relative mb-4 rounded overflow-hidden shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)]">
              <div 
                className="bg-yellow-400 h-full transition-all duration-300 ease-out" 
                style={{ width: `${signInProgress}%` }}
              />
            ...
            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider min-h-[1.5rem]">
              {signInStatus}
            </p>
          </div>
        </div>
          ...
          <header className="w-full sticky top-0 bg-white/95 backdrop-blur-sm border-b-4 border-black px-6 py-4 flex items-center justify-between z-40">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-900 border-2 border-black rounded text-[10px] sm:text-xs font-bold font-mono tracking-wider shadow-[2px_2px_0px_rgba(0,0,0,1)] transition-all active:translate-x-0.5 active:translate-y-0.5 active:shadow-none flex items-center gap-1.5"
              >
                <svg className="w-3.5 h-3.5" viewBox="0 0 24 24">
                  <path fill="#EA4335" d="M12 5.04c1.66 0 3.2.57 4.38 1.69l3.27-3.27C17.67 1.53 14.98 1 12 1 7.35 1 3.37 3.67 1.39 7.56l3.89 3.02c.92-2.75 3.49-4.54 6.72-4.54z" />
                  <path fill="#4285F4" d="M23.49 12.27c0-.81-.07-1.59-.2-2.36H12v4.51h6.46c-.29 1.48-1.14 2.73-2.4 3.58l3.73 2.89c2.18-2.01 3.7-4.99 3.7-8.62z" />
                  <path fill="#FBBC05" d="M5.28 14.78c-.24-.72-.38-1.48-.38-2.28s.14-1.56.38-2.28L1.39 7.2C.5 8.98 0 10.94 0 13s.5 4.02 1.39 5.8l3.89-3.02z" />
                  <path fill="#34A853" d="M12 23c3.24 0 5.97-1.07 7.96-2.91l-3.73-2.89c-1.12.75-2.54 1.19-4.23 1.19-3.23 0-5.8-1.79-6.72-4.54L1.39 16.9C3.37 20.79 7.35 23 12 23z" />
                </svg>
                <span className="hidden sm:inline">CONTINUE TO SAVE THE WORLD</span>
                <span className="sm:hidden">{simpleEnglish ? "EASY IN" : "SIGN IN"}</span>
              </button>
            </div>
          </header>
              ...
              <div className="lg:col-span-7 flex flex-col gap-4">
                ...
                <div className="mt-4 flex items-center gap-4">
                  ...
                  <button
                    onClick={handleGoogleSignIn}
                    onMouseEnter={() => sounds.hover()}
                    className="px-6 py-3 bg-white hover:bg-slate-100 text-slate-950 font-mono font-bold text-xs border-2 border-black rounded shadow-[4px_4px_0px_rgba(255,255,255,1)] active:translate-x-1 active:translate-y-1 active:shadow-none transition-all flex items-center gap-2"
                  >
                    <svg className="w-4 h-4 shadow-sm" viewBox="0 0 24 24">
                      <path fill="#EA4335" d="M12 5.04c1.66 0 3.2.57 4.38 1.69l3.27-3.27C17.67 1.53 14.98 1 12 1 7.35 1 3.37 3.67 1.39 7.56l3.89 3.02c.92-2.75 3.49-4.54 6.72-4.54z" />
                      <path fill="#4285F4" d="M23.49 12.27c0-.81-.07-1.59-.2-2.36H12v4.51h6.46c-.29 1.48-1.14 2.73-2.4 3.58l3.73 2.89c2.18-2.01 3.7-4.99 3.7-8.62z" />
                      <path fill="#FBBC05" d="M5.28 14.78c-.24-.72-.38-1.48-.38-2.28s.14-1.56.38-2.28L1.39 7.2C.5 8.98 0 10.94 0 13s.5 4.02 1.39 5.8l3.89-3.02z" />
                      <path fill="#34A853" d="M12 23c3.24 0 5.97-1.07 7.96-2.91l-3.73-2.89c-1.12.75-2.54 1.19-4.23 1.19-3.23 0-5.8-1.79-6.72-4.54L1.39 16.9C3.37 20.79 7.35 23 12 23z" />
                    </svg>
                    {simpleEnglish ? "Google Login: Save Earth" : "Continue to Save the World"}
                  </button>
                </div>
              </div>
                  ...
                  <div className="w-full h-full relative overflow-hidden rounded border border-slate-900 bg-slate-900">
                    {scrollSection < 10 ? (
                      ...
                      <div className="w-full h-full relative">
                        <ThreeEarth />
                        {/* Overlay with Google Login */}
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-[1px] p-4 text-center">
                          <div className="bg-slate-950/95 border-4 border-yellow-400 p-5 rounded-lg max-w-xs shadow-[0_0_20px_rgba(234,179,8,0.3)]">
                            <p className="font-mono text-[9px] text-yellow-400 font-bold uppercase tracking-wider mb-2 animate-pulse">
                              &gt;&gt; ORBITAL_LINK_SYNCHRONIZED
                            </p>
                            <h4 className="font-mono text-xs font-black text-white mb-4 uppercase">
                              {simpleEnglish ? "LOG IN TO SAVE THE PLANET" : "SUPABASE SECURE AUTH"}
                            </h4>
                            <button
                              onClick={handleGoogleSignIn}
                              className="w-full py-2.5 px-3 bg-white hover:bg-slate-100 text-slate-950 font-mono font-bold text-[10px] border-2 border-black rounded shadow-[2px_2px_0px_rgba(255,255,255,1)] transition-all active:translate-x-0.5 active:translate-y-0.5 active:shadow-none flex items-center justify-center gap-1.5 cursor-pointer"
                            >
                              <svg className="w-4 h-4 shadow-sm" viewBox="0 0 24 24">
                                <path fill="#EA4335" d="M12 5.04c1.66 0 3.2.57 4.38 1.69l3.27-3.27C17.67 1.53 14.98 1 12 1 7.35 1 3.37 3.67 1.39 7.56l3.89 3.02c.92-2.75 3.49-4.54 6.72-4.54z" />
                                <path fill="#4285F4" d="M23.49 12.27c0-.81-.07-1.59-.2-2.36H12v4.51h6.46c-.29 1.48-1.14 2.73-2.4 3.58l3.73 2.89c2.18-2.01 3.7-4.99 3.7-8.62z" />
          ...
          <section className="py-24 bg-yellow-400 text-black border-b-4 border-black relative overflow-hidden flex flex-col justify-center items-center text-center">
            ...
            <div className="container mx-auto px-6 relative z-10">
              ...
              <button
                onClick={handleGoogleSignIn}
                onMouseEnter={() => sounds.hover()}
                className="px-8 py-4 bg-white hover:bg-slate-100 text-slate-950 font-mono font-black text-sm sm:text-base border-4 border-black rounded-lg shadow-[6px_6px_0px_rgba(0,0,0,1)] hover:translate-y-[-2px] active:translate-y-[2px] transition-transform flex items-center gap-3 mx-auto"
              >
                ...
                <span>Continue to Save the World</span>
              </button>
            </div>
          </section>
